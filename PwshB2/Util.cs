...
namespace Ardalis.GuardClauses
{
    public static class B2ErrorGuard
    {
        ...
        public static void InvalidBucket(this IGuardClause guardClause, DtoBucket bucket)
        {
            ...
            {
                throw new Exception("A bucket's name cannot be less than 6 or more than 50 characters.");
            }
            ...
            {
                throw new Exception("You cannot have more than 10 bucket info entries.");
            }
            foreach (string key in bucket.bucketInfo.Keys)
            {
                if (key.Length > 25)
                {
                    throw new Exception("You cannot have a string greater than 25 characters in the bucket info.");
                }
            }
        ...
        public static void InvalidFile(this IGuardClause guardClause, DtoFile file)
        {
            ...
            {
                throw new Exception("File names cannot start with '/'.");
            }
            if (file.fileName.EndsWith("/"))
            {
                throw new Exception("File names cannot end with '/'.");
            }
            if (file.fileName.Contains("//"))
            {
                throw new Exception("File names cannot contain '//'.");
            }
            if (file.fileName.Contains("\\"))
            {
                throw new Exception("File names cannot contain '\'.");
            ...
            foreach (string section in file.fileName.Split('/'))
            {
                ...
                {
                    throw new Exception("A segment or name has exceeded 125 characters. A segment is the part between each '/'.");
                }
            }
            if (file.fileInfo.Count > 10)
            {
                throw new Exception("You cannot have more than 10 file info entries.");
            }
            foreach (string key in file.fileInfo.Keys)
            {
                if (key.Length > 25)
                {
                    throw new Exception("You cannot have a string greater than 25 characters in the file info.");
                }
